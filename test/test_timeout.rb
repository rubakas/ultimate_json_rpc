...
class TestRequestTimeout < Minitest::Test
  def test_slow_handler_times_out
    server = UltimateJsonRpc::Server.new(timeout: 0.1)
  ...
  def test_timeout_reader
    assert_equal 5, UltimateJsonRpc::Server.new(timeout: 5).timeout
  end

  def test_timeout_defaults_to_nil
    assert_nil UltimateJsonRpc::Server.new.timeout
  end

  def test_request_timeout_constant
    assert_equal(-32_001, UltimateJsonRpc::Core::REQUEST_TIMEOUT)
  end
