...
class TestRequestValidation < Minitest::Test
  def test_valid_request
    ...
    request = Reclamo::Core::Request.new(data)
  ...
  def test_rejects_non_hash_data
    assert_raises(Reclamo::Core::InvalidRequest) { Reclamo::Core::Request.new("string") }
    assert_raises(Reclamo::Core::InvalidRequest) { Reclamo::Core::Request.new([1, 2]) }
    assert_raises(Reclamo::Core::InvalidRequest) { Reclamo::Core::Request.new(nil) }
    assert_raises(Reclamo::Core::InvalidRequest) { Reclamo::Core::Request.new(42) }
  end

  def test_rejects_missing_jsonrpc
    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "method" => "foo", "id" => 1 })
    end
  end

  def test_rejects_wrong_jsonrpc_version
    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "1.0", "method" => "foo", "id" => 1 })
    end
  end

  def test_rejects_non_string_method
    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => 42, "id" => 1 })
    end
  end

  def test_rejects_empty_method
    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "", "id" => 1 })
    end
  end

  def test_rejects_missing_method
    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "id" => 1 })
    end
  end

  def test_rejects_invalid_params_type
    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "params" => "bad", "id" => 1 })
    end

    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "params" => 42, "id" => 1 })
    end
  end

  def test_rejects_invalid_id_types
    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "id" => [1] })
    end

    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "id" => { "x" => 1 } })
    end

    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "id" => true })
    end
  end

  def test_accepts_valid_id_types
    %w[string-id].each do |id|
      request = Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "id" => id })
  ...
  def test_accepts_null_id
    request = Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "id" => nil })
  ...
  def test_invalid_request_includes_descriptive_message
    err = assert_raises(Reclamo::Core::InvalidRequest) { Reclamo::Core::Request.new("string") }
    assert_match(/JSON object/, err.message)
  end

  def test_wrong_version_includes_descriptive_message
    err = assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "1.0", "method" => "foo" })
    end
  ...
  def test_invalid_params_type_includes_descriptive_message
    err = assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "params" => "bad" })
    end
...
class TestRequestNotification < Minitest::Test
  def test_request_with_id_is_not_notification
    request = Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "id" => 1 })

    refute_predicate request, :notification?
  end

  def test_request_without_id_is_notification
    request = Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo" })
...
class TestRequestFreezing < Minitest::Test
  ...
  def test_array_params_are_frozen
    request = Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "params" => [1, 2], "id" => 1 })

    assert_predicate request.params, :frozen?
  end

  def test_hash_params_are_frozen
    request = Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "params" => { "a" => 1 }, "id" => 1 })

    assert_predicate request.params, :frozen?
  end

  def test_id_is_frozen
    request = Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "foo", "id" => "abc" })
...
class TestRequestNestingLimit < Minitest::Test
  ...
  def test_nesting_beyond_max_raises
    ...
    assert_raises(Reclamo::Core::InvalidRequest) { Reclamo::Core::Request.new(data) }
  end
end
...
class TestRequestIdFalse < Minitest::Test
  def test_false_id_rejected
    assert_raises(Reclamo::Core::InvalidRequest) do
      Reclamo::Core::Request.new({ "jsonrpc" => "2.0", "method" => "test", "id" => false })
    end
  end
end
